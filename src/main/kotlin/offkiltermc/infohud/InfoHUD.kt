...
class InfoHUD : ModInitializer {
    ...
    override fun onInitialize() {
        ...
        S2CPlayChannelEvents.REGISTER.register(S2CPlayChannelEvents.Register { handler: ServerGamePacketListenerImpl, _: PacketSender?, _: MinecraftServer?, channels: List<ResourceLocation> ->
            handleRegistrationAdded(
                handler,
                channels
            )
        })
        S2CPlayChannelEvents.UNREGISTER.register(S2CPlayChannelEvents.Unregister { handler: ServerGamePacketListenerImpl, _: PacketSender?, _: MinecraftServer?, channels: List<ResourceLocation> ->
    ...
    private fun handleRegistrationAdded(handler: ServerGamePacketListenerImpl, channels: List<ResourceLocation>) {
        if (channels.contains(offkiltermc.infohud.InfoHUDNetworking.TICK_PERF)) {
        ...
        if (channels.contains(offkiltermc.infohud.InfoHUDNetworking.SERVER_LIGHT)) {
    ...
    private fun playerDisconnected(handler: ServerGamePacketListenerImpl, @Suppress("UNUSED_PARAMETER") server: MinecraftServer) {
    ...
    private fun serverTickEnd(server: MinecraftServer) {
        if (!tickPerfClients.isEmpty()) {
            ...
            if (now - lastTickPerfSend >= 1000) {
                ...
                buf.writeLong(server.tickTimesNanos.toList().average().toLong())
                buf.writeLong(tickCount.toLong())
                for (p in server.playerList.players) {
                    if (ServerPlayNetworking.canSend(p, offkiltermc.infohud.InfoHUDNetworking.TICK_PERF)) {
                        ServerPlayNetworking.send(p, offkiltermc.infohud.InfoHUDNetworking.TICK_PERF, buf)
                    }
                }
        ...
        if (!serverLightClients.isEmpty()) {
            ...
            if (now - lastServerLightSend >= 500) {
                ...
                if (level != null) {
                    ...
                    for (p in server.playerList.players) {
                        if (ServerPlayNetworking.canSend(p, offkiltermc.infohud.InfoHUDNetworking.SERVER_LIGHT)) {
                            ServerPlayNetworking.send(p, offkiltermc.infohud.InfoHUDNetworking.SERVER_LIGHT, buf)
                        }
                    }
                }
